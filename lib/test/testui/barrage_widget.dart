...
/// des: 弹幕组件
///
const Duration _kDuration = Duration(seconds: 20);
...
class BarrageWidget extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => BarrageWidgetState();
}

class BarrageWidgetState extends State<BarrageWidget> {
  ///
  /// 弹幕列表
  ///
  ...
  _computeTop(int index, double perRowHeight) {
    ...
    var top;
    int position = index % (widget.showCount);
    top = position * perRowHeight + widget.padding;
    ...
    // }
    // print("当前弹幕的高度top="+top.toString()+"|num="+num.toString()+"|perRowHeight="+perRowHeight.toString());
    return top;
  }
